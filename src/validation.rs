...
const CHECKS: [(&str, &str); 10] = [
    ...
    (
        ...
        "SELECT count(*)::bigint FROM (SELECT reconcile_account_id, external_reference FROM reconcile_external_transactions WHERE external_reference IS NOT NULL GROUP BY 1, 2 HAVING count(DISTINCT payload_hash) > 1) conflicts",
    ),
    (
        "run_completeness",
